...
public class CollectZone : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (m_collectionMode == CollectionMode.OnReleased)
        {
            m_inputs.Minigame1.Select.canceled += CollectOnRelease;
        }
    }
        private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.TryGetComponent(out Collectable collectable) && (CompareTag(collision.gameObject.tag) || CompareTag("Untagged")) )
        {
